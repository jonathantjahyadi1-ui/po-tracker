...
def status_label(value):
    return STATUS.get(value,LEGACY_STATUS.get(value,value))

def cell(value,link='',status=False,sub='',numeric=False):
    return {'value':status_label(value) if status else value,'link':link,'status':str(value) if status else '', 'sub':sub,'numeric':numeric}
